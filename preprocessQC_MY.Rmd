...
Different classes present in Minfi and their details:
1) RGChannelSet - Raw data from IDAT files, organized by probe color (green & red), instead of CpG sites. 
2) MethylSet - Raw data organized by the CpG locus, not mapped to genome. This data has two types: methylated and unmethylated probes
3) GenomicMethylSet - MethylSet data mapped to genome (hg19)
4) RatioSet - Raw data organized by the CpG locus, but not mapped to genome. The data contains beta values
5) GenomicRatioSet - RatioSet data mapped to genome (hg19)
